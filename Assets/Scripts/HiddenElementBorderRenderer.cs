...
public class HiddenElementBorderRenderer : MonoBehaviour
{
	public void Render(Match3Game game)
	{
		...
		{
			this.levelRendererPairs = new List<HiddenElementBorderRenderer.LevelRendererPair>();
			if (this.useHole && this.holeRenderer != null)
			{
				this.holeProvider = new HiddenElementBorderRenderer.HoleProvider();
				this.holeProvider.Init(game);
				HiddenElementBorderRenderer.LevelRendererPair levelRendererPair = new HiddenElementBorderRenderer.LevelRendererPair();
			...
			for (int i = 0; i < 2; i++)
			{
				HiddenElementBorderRenderer.LevelRendererPair levelRendererPair2 = new HiddenElementBorderRenderer.LevelRendererPair();
				HiddenElementBorderRenderer.LevelSlotsProvider levelSlotsProvider = new HiddenElementBorderRenderer.LevelSlotsProvider();
		...
		{
			HiddenElementBorderRenderer.LevelRendererPair levelRendererPair3 = this.levelRendererPairs[j];
			if (!levelRendererPair3.isHidden)
			{
				HiddenElementBorderRenderer.HiddenElementProvider levelProvider = levelRendererPair3.levelProvider;
	...
	private HiddenElementBorderRenderer.HoleProvider holeProvider;

	private List<HiddenElementBorderRenderer.LevelRendererPair> levelRendererPairs;
	...
	public class LevelSlotsProvider : HiddenElementBorderRenderer.HiddenElementProvider
	{
	...
	public class HoleProvider : HiddenElementBorderRenderer.HiddenElementProvider
	{
		public int CountColoredSlates()
		{
	...
	{
		public HiddenElementBorderRenderer.HiddenElementProvider levelProvider;
