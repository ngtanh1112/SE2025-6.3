...
public class CarDragButton : MonoBehaviour, IPointerDownHandler, IEventSystemHandler, IPointerUpHandler
{
	...
	{
		return new CarDragButton._003CDoAnimate_003Ed__15(0)
		{
	...
	private sealed class _003CDoAnimate_003Ed__15 : IEnumerator<object>, IEnumerator, IDisposable
	{
		...
		bool IEnumerator.MoveNext()
		{
			...
			CarDragButton carDragButton = this._003C_003E4__this;
			if (num != 0)
			{
				if (num != 1)
				{
			...
			float num2 = Mathf.PingPong(this._003Ctime_003E5__2, carDragButton.settings.bobDuration);
			num2 = carDragButton.settings.animationCurve.Evaluate(num2);
			...
			float d = Mathf.LerpUnclamped(0f, carDragButton.settings.bobDisplace, num2);
